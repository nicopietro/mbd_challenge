...
@app.post('/api/v1/mpc/train', tags=['Machine Learning'], summary='Train a new model', description='Trains a new model using simulated animal data and saves it to MinIO.')
def traing_new_model(datapoints: int, seed: int=42):
    ...
    if datapoints < 1:
        return JSONResponse(status_code=422, content={'error': 'Number of datapoints must be greater than 0.'})
    ...
    return JSONResponse(status_code=200, content={'status': 'Ok', 'trained_model_id': timestamp, 'model_metrics': metrics})

@app.get('/health', tags=['Health'], summary='Health check', description='Checks the health of the service and its dependencies (MinIO and Data Service API).')
def health_check():
    """
